...
export const DiffLine = styled.div<{ type: string; $isWordMode?: boolean }>`
  ...
  ${({ type, theme, $isWordMode }) => {
    if ($isWordMode) return css`background: transparent; color: ${theme.colors.text};`;
...
export const DiffLineContent = styled.span<{ type?: string }>`
  padding: 2px 0;
  width: 100%;
...
export const WordHighlight = styled.span<{ $type: 'added' | 'removed' | 'modified' | 'unchanged' }>`
  ${({ $type, theme }) => {
    switch ($type) {
      case 'added':
        return css`
          background-color: ${theme.colors.diffAddedText};
          color: #fff;
          border-radius: 2px;
          padding: 0 2px;
        `;
      case 'removed':
        return css`
          background-color: ${theme.colors.diffRemovedText};
          color: #fff;
          border-radius: 2px;
          padding: 0 2px;
      ...
      case 'modified':
        return css`
          background-color: ${theme.colors.diffChangedText};
          color: #fff;
          border-radius: 2px;
          padding: 0 2px;
        `;
